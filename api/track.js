...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enriched = {
      _subject: `Evidence Axis tracking v4 — ${body.company || 'Unknown company'}`,
      _template: 'table',
    ...
    // Keep a structured server-side record in Vercel runtime logs even if email forwarding fails.
    ...
    try {
      ...
      if (!forwarded) console.error('EA_TRACKING_FORWARD_FAILED', upstream.status, enriched.delivery_token, enriched.event);
    } catch (forwardError) {
      console.error('EA_TRACKING_FORWARD_ERROR', String(forwardError), enriched.delivery_token, enriched.event);
    }
